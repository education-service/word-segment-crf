...
public class Trainer {
	...
	protected void computeFeatureExpectedValue(DataIter dataIter, FeatureGenerator fgen, double lambda[],
			double expFVals[]) {
		diter = dataIter;
		featureGenCache = null;
	...
	protected double computeFunctionGradient(double lambda[], double grad[], double expFVals[],
		...
		try {
			...
			if (params.debugLvl > 0) {
				...
				if (grad != null)
					Util.printDbg("Iter " + icall + " loglikelihood " + logli + " gnorm " + norm(grad) + " xnorm "
							+ norm(lambda));
			}
			return logli;
		} catch (Exception e) {
	...
	protected double sumProduct(DataSequence dataSeq, FeatureGenerator featureGenerator, double lambda[],
		...
		if (logProcessing) {
			return sumProductLL(dataSeq, featureGenerator, lambda, grad, expFVals, onlyForwardPass, numRecord,
					fgenForExpVals);
		}
	...
	static boolean computeLogMiInitDone(FeatureGenerator featureGen, double lambda[], DoubleMatrix2D Mi_YY,
	...
	public static double initLogMi(double defaultValue, Iterator constraints, DoubleMatrix2D Mi, DoubleMatrix1D Ri) {
		if (constraints != null) {
	...
	static boolean computeLogMi(FeatureGenerator featureGen, double lambda[], DoubleMatrix2D Mi_YY,
	...
	public static void computeLogMi(FeatureGenerator featureGen, double lambda[], DataSequence dataSeq, int i,
	...
	public static boolean computeLogMi(FeatureGenerator featureGen, double lambda[], DataSequence dataSeq, int i,
			DoubleMatrix2D Mi_YY, DoubleMatrix1D Ri_Y, boolean takeExp, boolean reuseM, boolean initMDone) {
	...
	protected double sumProductLL(DataSequence dataSeq, FeatureGenerator featureGenerator, double lambda[],
		...
		double thisSeqLogli = sumProductInner(dataSeq, featureGenerator, lambda, gradThisInstance, onlyForwardPass,
				numRecord, ((grad != null) || (expFVals != null)) ? fgenForExpVals : null);
	...
	protected void getMarginals(DataSequence dataSeq, FeatureGenerator featureGenerator, double lambda[],
			float nodeMargs[][], float edgeMargs[][][]) {
		allocateAlphaBeta(2 * dataSeq.length() + 1);
		beta_Y = computeBetaArray(dataSeq, lambda, featureGenerator);
		alpha_Y.assign(0);
		for (int i = 0; i < dataSeq.length(); i++) {
			// compute the Mi matrix
			initMDone = computeLogMiTrainMode(featureGenerator, lambda, dataSeq, i, Mi_YY, Ri_Y, false, reuseM,
					initMDone);
			...
			for (int y = 0; y < numY; y++) {
				...
				if (i > 0) {
					for (int yprev = 0; yprev < numY; yprev++) {
						edgeMargs[i][yprev][y] = (float) (alpha_Y.get(yprev) + beta_Y[i].get(y) + Ri_Y.get(y) + Mi_YY
								.get(yprev, y));
					}
				}
			}
				...
				while (fgenForExpVals.hasNext()) {
					...
					if (yprev < 0) {
						ExpF[f] = RobustMath.logSumExp(ExpF[f],
								newAlpha_Y.get(yp) + RobustMath.log(val) + beta_Y[i].get(yp));
					} else {
						ExpF[f] = RobustMath.logSumExp(ExpF[f],
								alpha_Y.get(yprev) + Ri_Y.get(yp) + Mi_YY.get(yprev, yp) + RobustMath.log(val)
										+ beta_Y[i].get(yp));
					}
				}
			}
	...
	protected boolean computeLogMiTrainMode(FeatureGenerator featureGen, double lambda[], DataSequence dataSeq, int i,
			DoubleMatrix2D Mi_YY, DoubleMatrix1D Ri_Y, boolean takeExp, boolean reuseM, boolean initMDone) {
