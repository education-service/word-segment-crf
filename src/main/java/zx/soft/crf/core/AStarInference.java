...
import zx.soft.crf.astar.AStarSearch;
import zx.soft.crf.astar.State;
import zx.soft.crf.core.Viterbi.Entry;
...
public class AStarInference implements Serializable {
	...
	protected void getParameters() {
		...
		if (model.params.miscOptions.getProperty("avgStatesPerExpansion") != null) {
			try {
				avgStatesPerExpansion = Integer
						.parseInt(model.params.miscOptions.getProperty("avgStatesPerExpansions"));
			} catch (NumberFormatException nfe) {
			}
		}
		...
		if (model.params.miscOptions.getProperty("forwardViterbiBeamSize") != null) {
			try {
				forwardViterbiBeamSize = Integer.parseInt(model.params.miscOptions
						.getProperty("forwardViterbiBeamSize"));
			} catch (NumberFormatException nfe) {
			}
		}
		...
		if (model.params.miscOptions.getProperty("backwardViterbiBeamSize") != null) {
			try {
				backwardViterbiBeamSize = Integer.parseInt(model.params.miscOptions
						.getProperty("backwardViterbiBeamSize"));
			} catch (NumberFormatException nfe) {
			}
		}
	...
	public AStarInference(CRF model, int bs, TIntObjectHashMap<TIntHashSet> confLabelMap, Model graphModel) {
		this.model = model;
	...
	public double aStarSearch(DataSequence dataSeq, double lambda[], boolean calcCorrectScore) {
	...
	private void computeMi(DataSequence dataSeq, double lambda[]) {
		...
		for (int pos = 0; pos < seqLength; pos++) {
			// compute Mi.
			Trainer.computeLogMi(model.featureGenerator, lambda, dataSeq, pos, Mi[pos], Ri[pos], false);
		}
	}
	...
	private AStarState getStartState() {
		return (conflictingLabels == null ? new AStarState(-1, -1, upperBound, 0, null, null) : new AStarState(-1, -1,
				upperBound, 0, null, new BitSet()));
	}
	...
	class AStarState extends State {
		...
		public AStarState(int pos, int label, double h, double g, AStarState predecessor, BitSet assignedLabels) {
			super(g, h);
			this.pos = pos;
			...
			for (int nextY = 0; nextY < model.numY; nextY++) {
				...
				if (assignedLabels != null) {
					if (!conflicting(succLabels, nextY))
						successors[nextY] = new AStarState(nextPos, nextY, ubScore[nextPos][nextY], succScore, this,
								succLabels);//todo
					else
						successors[nextY] = null;//invalid state
				} else
					successors[nextY] = new AStarState(nextPos, nextY, ubScore[nextPos][nextY], succScore, this, null);
			}
			return successors;
		}
		...
		public boolean conflicting(BitSet assignedLabels, int stateId) {
			if (constraintCheck && assignedLabels != null && conflictingLabels.get(graphModel.label(stateId)) != null) {
				return (assignedLabels.intersects(conflictingLabels.get(graphModel.label(stateId))));
			}
			return false;
		}
		...
		public String toString() {
			return "Pos:" + pos + " Y=" + graphModel.label(y) + " h=" + h + " g=" + g + " f=" + (h + g)
					+ (predecessor != null ? " Par=(" + predecessor.pos + ", " + predecessor.y + ")" : "");
		}
