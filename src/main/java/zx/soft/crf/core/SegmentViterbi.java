...
public class SegmentViterbi extends SparseViterbi {
	...
	public static class LabelConstraints {
		...
		public boolean contained(TIntHashSet labelsOnPath, TIntHashSet prevLabels) {
			if (labelsOnPath == null)
				return true;
			for (TIntIterator iter = labelsOnPath.iterator(); iter.hasNext();) {
				int thisL = iter.next();
				if (prevLabels != null && prevLabels.contains(thisL))
					continue;
				return false;
			}
			return true;
	...
	public class SolnWithLabelsOnPath extends Soln {
		...
		protected void copy(Soln soln) {
			...
			for (TIntIterator iter = ((SolnWithLabelsOnPath) soln).labelsOnPath.iterator(); iter.hasNext(); labelsOnPath
					.add(iter.next()))
				;
		}
		...
		public void setPrevSoln(Soln prevSoln, float score) {
			...
			if ((prevSoln != null) && (labelConstraints != null)) {
				labelConstraints.formPreviousLabel(((SolnWithLabelsOnPath) prevSoln).labelsOnPath, labelsOnPath,
						prevSoln.label);
			}
		}
	}
	...
	public class EntryForLabelConstraints extends Entry {
		...
		protected int findInsert(int insertPos, float score, Soln prev) {
			...
			TIntHashSet prevLabels = ((prevSolL != null && labelConstraints != null) ? labelConstraints
					.formPreviousLabel(prevSolL.labelsOnPath, tmpLabels, prevSolL.label) : null);
			...
			for (insertPos = 0; insertPos < size(); insertPos++) {
				if (score > get(insertPos).score) {
					...
					if (labelConstraints != null
							&& labelConstraints.contained(prevLabels,
									((SolnWithLabelsOnPath) get(insertPos)).labelsOnPath)) {
		...
		public void add(int y, Entry prevSoln, float thisScore) {
			if (labelConstraints == null) {
				...
				if (getQuick(y) == null) {
					setQuick(
							y,
							new EntryForLabelConstraints((pos == startPos) ? 1 : beamsize, y, pos, labelConstraints
									.countConflicting(size())));
				}
				super.add(y, prevSoln, thisScore);
			}
		}
	...
	protected Iter getIter() {
		return (markovModel ? new Iter() : new SegmentIter());
	}
	...
	protected double getCorrectScore(DataSequence dataSeq, int i, int ell, double[] lambda) {
		...
		if ((labelConstraints != null) && labelConstraints.conflicting(data.y(i))) {
			for (int segStart = 0; segStart < i - ell + 1; segStart = data.getSegmentEnd(segStart) + 1) {
				...
				if (labelConstraints.disallowedPairs.conflictingPair(data.y(i), data.y(segStart),
						(segEnd == i - ell) ? -1 : 0)) // TODO: 0 here is not correct.
					return RobustMath.LOG0;
			}
		}
		if (model.params.debugLvl > 1) {
			...
			while (featureGenNested.hasNext()) {
				...
				if (((CandSegDataSequence) data).holdsInTrainingData(f, i - ell, i)) {
					System.out.println("Feature " + (i - ell) + " " + i + " " + featureGenNested.featureName(f.index())
							+ " " + lambda[f.index()] + " " + f.value());
				}
			}
		}
	...
	public double viterbiSearch(DataSequence dataSeq, double lambda[], DoubleMatrix2D[][] Mis, DoubleMatrix1D[][] Ris,
	...
	public double sumScoreTopKViolators(DataSequence dataSeq, double lambda[]) {
		...
		for (int k = numSols - 1; k >= 0; k--) {
			...
			if (violating)
				totalScore = RobustMath.logSumExp(score, totalScore);
		}
	...
	@Override
	public double viterbiSearchBackward(DataSequence dataSeq, double[] lambda, DoubleMatrix2D Mis[][],
			DoubleMatrix1D Ris[][], boolean calcCorrectScore) {
		labelConstraints = null;
		return super.viterbiSearchBackward(dataSeq, lambda, Mis, Ris, calcCorrectScore);
	}

	public double viterbiSearchBackward(DataSequence dataSeq, double[] lambda, DoubleMatrix2D Mis[][],
			DoubleMatrix1D Ris[][], boolean constraints, boolean calcCorrectScore) {
		if (constraints)
			labelConstraints = LabelConstraints.checkConstraints((CandSegDataSequence) dataSeq, labelConstraints);
		else
			labelConstraints = null;
