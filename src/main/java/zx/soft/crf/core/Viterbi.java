...
public class Viterbi implements Serializable {
	...
	protected class Entry {
		...
		public String toString() {
			...
			if (solns[0].prevSoln != null)
				toString += " : " + solns[0].prevSoln.pos + " " + solns[0].prevSoln.label + " "
						+ solns[0].prevSoln.score;
			toString += "]";
			return toString;
		}
	...
	double fillArrayBackward(DataSequence dataSeq, double lambda[], Entry firstEntries[], DoubleMatrix2D Mis[],
			DoubleMatrix1D Ris[], boolean calcScore) {
		double corrScore = 0;
		int numY = model.numY;
	...
	public LabelSequence[] topKLabelSequences(DataSequence dataSeq, double[] lambda, int numLabelSeqs, boolean getScores) {
		...
		for (int k = numSols - 1; k >= 0; k--) {
			...
			if (getScores)
				labelSequences[k].score = Math.exp(ybest.score - lZx);
			ybest = ybest.prevSoln;
			while (ybest != null) {
