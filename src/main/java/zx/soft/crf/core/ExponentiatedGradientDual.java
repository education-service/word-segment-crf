...
/**
 * Trainer as in "Exponentiated Gradient Algorithms for Conditional Random Fields and Max-Margin Markov Networks" in JMLR 08
 */
public class ExponentiatedGradientDual extends Trainer {
				...
				for (int y = 0; y < numY; y++) {
					...
					if (i > 0) {
						for (int yprev = 0; yprev < numY; yprev++) {
							thetaTransition[k][i][yprev][y] -= eta
									* (thetaTransition[k][i][yprev][y] - params.invSigmaSquare * Mi_YY.get(yprev, y));
						}
					}
				}
			}
	...
	private void calculateExpFValuesUsingTheta(DataSequence dataSeq, double[] expF, double[][] thetaStateK,
			double[][][] thetaEdgeK, boolean addCorrFVec) {
		...
		for (int i = 0; i < dataSeq.length(); i++) {
			...
			while (featureGenerator.hasNext()) {
				...
				if (yprev < 0) {
					...
					expF[f] = RobustMath.logSumExp(ExpF[f],
							alpha_Y.get(yprev) + thetaStateK[i][yp] + tempMi_YY.get(yprev, yp) + RobustMath.log(val)
									+ beta_Y[i].get(yp));
				}
			}
