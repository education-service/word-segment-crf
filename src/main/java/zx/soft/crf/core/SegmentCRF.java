...
public class SegmentCRF extends CRF {
	...
	public double segmentMarginalProbabilities(DataSequence dataSequence, TIntDoubleHashMap segmentMarginals[][],
		...
		return -1
				* ((SegmentTrainer) trainer).sumProductInner(dataSequence, featureGenerator, lambda, null, false, -1,
						null, segmentMarginals, edgeMarginals);
	}
	...
	public double[] marginalProbsOfSegmentation(DataSequence dataSequence, Segmentation segmentation) {
		...
		for (int i = segmentation.numSegments() - 1; i >= 0; i--) {
			margPr[i] = segMargs[segmentation.segmentLabel(i)][segmentation.segmentStart(i)].get(segmentation
					.segmentEnd(i));
		}
		return margPr;
	}
