...
import zx.soft.crf.core.Trainer.SumFunc;
...
public class RobustMath {
	...
	static class LogExpCache {
		...
		static double lookupAddWorse(double val) {
			...
			System.out.println((retval - Math.log(Math.exp(-1 * val) + 1.0)) + " "
					+ (lookupAdd(val) - Math.log(Math.exp(-1 * val) + 1.0)));
			return retval;
		}
	};
	...
	public static DoubleMatrix1D logMult(DoubleMatrix2D M, DoubleMatrix1D y, DoubleMatrix1D z, double alpha,
	...
	public static DoubleMatrix1D logMult(DoubleMatrix2D M, DoubleMatrix1D y, DoubleMatrix1D z, double alpha,
			double beta, boolean transposeA, EdgeGenerator edgeGen) {
		// z = alpha * A * y + beta*z
		// in log domain this becomes:
		...
		for (int j = 0; j < M.columns(); j++) {
			for (int i = (edgeGen == null ? j : edgeGen.first(j)); i < M.rows(); i = (edgeGen == null) ? i + 1
					: edgeGen.next(j, i)) {
				int r = i;
				int c = j;
				if (transposeA) {
	...
	static DoubleMatrix1D Mult(DoubleMatrix2D M, DoubleMatrix1D y, DoubleMatrix1D z, double alpha, double beta,
			boolean transposeA, EdgeGenerator edgeGen) {
		// z = alpha * A * y + beta*z
	...
	public static void logMatrixMult(DoubleMatrix2D result, DoubleMatrix2D A, DoubleMatrix2D B, DoubleMatrix1D ri,
			boolean noMatrixMult) {
