...
public class SoftLogMarginTrainer extends Trainer {
	...
	protected double sumProductInner(DataSequence dataSeq, FeatureGenerator featureGenerator, double[] lambda,
		...
		if (initPhase)
			return super.sumProductInner(dataSeq, featureGenerator, lambda, grad, onlyForwardPass, numRecord,
					fgenForExpVals);
		...
		for (int i = 0; i < dataSeq.length(); i++) {
			...
			if (i > 0) {
				...
				double t2 = RobustMath.logSumExp(RobustMath.logSumExp(tmp_Y), beta_Y[0].get(ycorr) + Ri_Y.get(ycorr))
						- trainWDotF;
				betat1 = t1;
				betat2 = t2;
				try {
			...
			while (fgenForExpVals.hasNext()) {
				...
				if (yprev < 0) {
					...
					logt2 = RobustMath.logSumExp(alphaLoss[i].get(yp) + beta_Y[i].get(yp), alphas[i].get(yp)
							+ betaLoss[i].get(yp));
				} else {
	...
	protected double sumProductInnerOld(DataSequence dataSeq, FeatureGenerator featureGenerator, double[] lambda,
			double[] grad, boolean onlyForwardPass, int numRecord, FeatureGenerator fgenForExpVals) {
		if ((beta_Y == null) || (beta_Y.length < dataSeq.length())) {
			beta_Y = new DenseDoubleMatrix1D[2 * dataSeq.length()];
			for (int i = 0; i < beta_Y.length; i++)
