...
public class SparseViterbi extends Viterbi {
	...
	protected class ContextUpdate implements IntIntDoubleFunction, IntDoubleFunction {
		...
		public double fillArray(DataSequence dataSeq, double[] lambda, DoubleMatrix2D[][] Mis, DoubleMatrix1D[][] Ris,
				boolean calcScore) {
			...
			while (!stack.empty()) {
				...
				default:
					if (context[soln.prevPos()].entryNotNull(soln.prevLabel()))
						context[soln.pos].add(soln.label, context[soln.prevPos()].getEntry(soln.prevLabel()),
								soln.score - soln.prevSoln.score);
					break;
				}
		...
		private double fillArray(DataSequence dataSeq, double[] lambda, DoubleMatrix2D[][] Mis, DoubleMatrix1D[][] Ris,
	...
	class BackwardContextUpdate implements IntIntDoubleFunction, IntDoubleFunction {
		...
		double fillArray(DataSequence dataSeq, double lambda[], DoubleMatrix2D Mis[][], DoubleMatrix1D Ris[][],
				boolean calcScore) {
