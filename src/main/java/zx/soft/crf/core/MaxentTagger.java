...
public class MaxentTagger extends CRF {
	 ...
	 * @param configOptions
	 */
	public MaxentTagger(int numLabels, FeatureGenerator fgen, Properties configOptions) {
	 ...
	 * @param configOptions
	 */
	public MaxentTagger(int numLabels, int histsize, FeatureGenerator fgen, Properties configOptions) {
	...
	class MaxentViterbi extends Viterbi {
		...
		@Override
		public LabelSequence[] topKLabelSequences(DataSequence dataSeq, double[] lambda, int numLabelSeqs,
				boolean getScores) {
			...
			for (int k = numSols - 1; k >= 0; k--) {
				...
				if (getScores)
					labelSequences[k].score = Math.exp(labelSequences[k].score - model.getLogZx(dataSeq));
			}
