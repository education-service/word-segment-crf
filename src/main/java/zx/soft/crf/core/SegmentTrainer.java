...
public class SegmentTrainer extends SparseTrainer {
	...
	@Override
	protected double sumProductInner(DataSequence data, FeatureGenerator featureGenerator, double lambda[],
			double grad[], boolean onlyForwardPass, int numRecord, FeatureGenerator fgenForExpCompute) {
		return sumProductInner(data, featureGenerator, lambda, grad, onlyForwardPass, numRecord, fgenForExpCompute,
				null, null);
	}
		...
		if (!onlyForwardPass) {
			...
			for (int segEnd = dataSeq.length() - 1; segEnd >= 0; segEnd--) {
				...
				for (int nc = 0; nc <= numCands; nc++) {
					...
					initMDone = computeLogMi(dataSeq, i, i + ell, featureGenNested, lambda, Mi_YY, Ri_Y, reuseM,
							initMDone);
					tmp_Y.assign(Ri_Y);
					if (i + ell < dataSize - 1)
		...
		for (int segEnd = 0; segEnd < dataSize; segEnd++) {
			...
			if ((grad != null) && (trainingSegmentEnd < segEnd)) {
				if ((!trainingSegmentFound) && noneFired) {
					System.out.println("Error: Training segment (" + trainingSegmentStart + " " + trainingSegmentEnd
							+ ") not found amongst candidate segments");
				}
			...
			for (int nc = 0; nc <= numCands; nc++) {
				...
				// compute the Mi matrix
				initMDone = computeLogMi(dataSeq, segEnd - ell, segEnd, featureGenNested, lambda, Mi_YY, Ri_Y, reuseM,
						initMDone);
				...
				if (segEnd - ell >= 0) {
					if (!reuseM)
						...
						if (!initAlphaMDone[segEnd - ell - base]) {
							...
							Mi_YY.zMult(alpha_Y_Array[segEnd - ell - base], alpha_Y_ArrayM[segEnd - ell - base], 1, 0,
									true);
					...
					while (featureGenNestedForExpVals.hasNext()) {
						...
						if (yprev < 0) {
							ExpF[f] = RobustMath.logSumExp(ExpF[f],
									(newAlpha_Y.get(yp) + RobustMath.log(val) + beta_Y[segEnd].get(yp)));
						} else {
							ExpF[f] = RobustMath.logSumExp(
									ExpF[f],
									(alpha_Y_Array[segEnd - ell - base].get(yprev) + Ri_Y.get(yp)
											+ Mi_YY.get(yprev, yp) + RobustMath.log(val) + beta_Y[segEnd].get(yp)));
						}
					}
				}
				...
				if (segmentMarginals != null) {
					for (int yp = newAlpha_Y.size() - 1; yp >= 0; yp--) {
						if ((segmentMarginals[yp][segEnd - ell + 1] != null)
								&& (segmentMarginals[yp][segEnd - ell + 1].containsKey(segEnd))) {
							// segmentMarginals[yp][segEnd-ell+1] = new TIntDoubleHashMap();
							segmentMarginals[yp][segEnd - ell + 1].put(segEnd,
									newAlpha_Y.get(yp) + beta_Y[segEnd].get(yp));
						...
						if (edgeMarginals != null) {
							for (int yprev = newAlpha_Y.size() - 1; yprev >= 0; yprev--) {
								...
								edgeMarginals[yprev][yp][segEnd - ell + 1].put(segEnd, alpha_Y_Array[segEnd - ell
										- base].get(yprev)
										+ Ri_Y.get(yp) + Mi_YY.get(yprev, yp) + beta_Y[segEnd].get(yp));
				...
				if ((grad != null) && (segEnd == trainingSegmentEnd) && (segEnd - ell + 1 == trainingSegmentStart)) {
					...
					if ((val1 == RobustMath.LOG0) || (val2 == RobustMath.LOG0)) {
						System.out.println("Error: training labels not covered in generated features " + val1 + " "
								+ val2 + " y " + dataSeq.y(trainingSegmentEnd));
			...
			if (normalize) {
				for (int y = 0; y < segmentMarginals.length; y++) {
					for (int segStart = 0; segStart < segmentMarginals[y].length; segStart++) {
						if (segmentMarginals[y][segStart] == null)
							continue;
						for (TIntDoubleIterator segEndProbIter = segmentMarginals[y][segStart].iterator(); segEndProbIter
								.hasNext();) {
						...
						if (edgeMarginals != null) {
							for (int yprev = 0; yprev < edgeMarginals.length; yprev++) {
								for (TIntDoubleIterator segEndProbIter = edgeMarginals[yprev][y][segStart].iterator(); segEndProbIter
										.hasNext();) {
									segEndProbIter.advance();
									segEndProbIter.setValue(Math.exp(segEndProbIter.value() - lZx));
