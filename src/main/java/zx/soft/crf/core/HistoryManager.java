...
import zx.soft.crf.utils.Counters;
...
class HistoryManager implements Serializable {
	...
	class FeatureHist implements Feature {
		...
		void print() {
			System.out.println(type() + index() + " " + y() + " " + yprev() + " " + value());
		}
	};
	...
	class FeatureGeneratorWithHistory implements FeatureGenerator {
		...
		zx.soft.crf.model.FeatureImpl feature = new zx.soft.crf.model.FeatureImpl();
	...
	class DataSequenceHist implements DataSequence {
		...
		DataSequenceHist() {
			cntr = new Counters(histsize, numOrigY);
		}
