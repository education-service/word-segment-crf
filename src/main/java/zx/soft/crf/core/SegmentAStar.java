...
import zx.soft.crf.astar.BoundUpdate;
import zx.soft.crf.astar.State;
import zx.soft.crf.core.SegmentViterbi.LabelConstraints;
import zx.soft.crf.core.SparseViterbi.Iter;
import zx.soft.crf.utils.OptimizedSparseDoubleMatrix1D;
import zx.soft.crf.utils.OptimizedSparseDoubleMatrix2D;
import zx.soft.crf.utils.StaticObjectHeap;
...
public class SegmentAStar extends AStarInference {
	...
	public SegmentAStar(SegmentCRF model, int bs) {
		...
		this.model = model;
		getParameters();
		aStar = new AStarSearch((boundUpdate ? new AStarBoundUpdate() : null), avgStatesPerExpansion, maxExpansions,
				queueSizeLimit, debug);
	...
	public double aStarSearch(DataSequence dataSeq, double lambda[], boolean calcCorrectScore, boolean checkConstraints) {
	...
	private AStarState getStartState() {
		return new SegmentState(-1, 0, -1, upperBound, 0, null, (labelConstraints != null ? new CloneableIntSet()
				: null));
	}
	...
	class SegmentState extends AStarState implements OptimizedSparseDoubleMatrix1D.ForEachNonZeroReadOnly {
		...
		public SegmentState(int pos, int ell, int label, double h, double g, AStarState predecessor,
				CloneableIntSet labelsOnPath) {
		...
		public String toString() {
			return id + ">Pos:" + pos + " ell=" + ell + " Label=" + y + " score="
					+ (g - (predecessor != null ? predecessor.g() : 0)) + " g=" + g + " h=" + h + " f=" + (h + g)
