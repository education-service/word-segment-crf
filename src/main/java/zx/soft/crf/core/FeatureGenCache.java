...
public class FeatureGenCache implements FeatureGeneratorNested {
	...
	public static class AllFeatureCache {
		...
		class FeatureImpl implements Feature {
			...
			public boolean equals(Object obj) {
				...
				if (obj == null)
					return false;
				return (allButValueEqual(obj) && (Math.abs(_value - ((Feature) obj).value()) < Float.MIN_VALUE));
			}
		}
		...
		class FeatureCacheWithYPrev extends FeatureImplWithYPrev {
			...
			public FeatureCacheWithYPrev(Feature f) {
				super(f);
			}
		...
		public Flist newFlist(int numLabels) {
			return new Flist(numLabels);
		}
	}
	...
	public void startDataScan() {
		...
		if (scanNum == 2) {
			...
			System.out.println("First scan done..distinct features "
					+ (featureCache.featureVariants.size() + featureCache.distinctFeatures.size()));
		}
	}
	...
	class Cursor {
		...
		public Feature nextFeature() {
			int featureId = (currentFeatureOffset < featureOffsetEnd) ? featureIds.get(currentFeatureOffset++)
					: edgeFeatureIds.get(edgeFeatureId--);
