...
public class Counters {
	...
	int nextNonFixed(int i) {
		return fixedVals.nextClearBit(i);
	}

	public boolean isFixed(int index) {
		return fixedVals.get(index);
	}
	...
	public boolean done() {
		return (cnts[cnts.length - 1] >= maxVals[cnts.length - 1]);
	}
