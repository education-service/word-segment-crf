...
public class BWindowFeatureMultiNew extends WindowFeatures implements BoundaryFeatureFunctions {
	...
	private void initBoundary(Window twindow) {
		...
		if (posBdry.start > posBdry.end || featureBdry.start > featureBdry.end) {
			if (DEBUG)
				System.out.println("TODO : check the initBoundary method : " + posBdry + " : " + featureBdry);
		}
	...
	public boolean hasNext() {
		return directSegmentMode ? super.hasNext() : allFeaturesFlag == false;
	}
		...
		for (int i = 0; i < twindowArr.length; i++) {
			...
			case WIN_RIGHT:
				if (twindow.maxLength < Integer.MAX_VALUE) {
					System.out
							.println("BWindowFeatureMulti : The maxLength of windows defined for either end can not be less than Integer.MAX_VALUE");
					return false;
				}
				break;
			case WIN_MIDDLE:
