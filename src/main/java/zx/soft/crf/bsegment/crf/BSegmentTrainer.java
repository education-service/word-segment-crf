...
import zx.soft.crf.core.SegmentTrainer;
...
class BSegmentTrainer extends SegmentTrainer {
	...
	private double computeFeatureGradOpt(FeatureStore.Iter fIter, DataSequence dataSeq, int m, double grad[],
			double lambda[], double thisSeqLogli) {
		if (!fIter.hasNext())
			return thisSeqLogli;
		...
		for (int s = 0; s < dataSize; s++) {
			...
			for (e--; e >= s; e--) {
				...
				if (params.debugLvl > 2)
					System.out.println("Features for boundary: [" + s + " " + e + "]");
				while ((f != null) && ((f.start() == s) && (f.end() == e))) {
					if ((icall == 0) && (grad != null) && holdsInTrainingData(f, (Segmentation) dataSeq)) {
						F[f.index()] += f.value();

						if (params.debugLvl > 2)
							System.out.println("Holds " + f.index() + " " + bfgen.featureName(f.index()) + " "
									+ f.start() + " " + f.startOpen() + " " + f.end());
					}
	...
	protected double sumProductInner(DataSequence dataSeq, FeatureGenerator featureGenerator, double lambda[],
		...
		for (int i = dataSeq.length() - 2; i >= 0; i--) {
			...
			for (int ip = i + 1; (ip <= i + m) && (ip < dataSize); ip++) {
				...
				tmp_Y.assign(Ri_Y);
				if (ip < dataSize - 1)
					tmp_Y.assign(beta_Y[ip], sumFunc);
