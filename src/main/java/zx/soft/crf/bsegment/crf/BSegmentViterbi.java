...
import zx.soft.crf.core.LabelSequence;
import zx.soft.crf.core.LogSparseDoubleMatrix1D;
import zx.soft.crf.core.SegmentViterbi;
import zx.soft.crf.core.Segmentation;
import zx.soft.crf.core.Soln;
import zx.soft.crf.core.SparseViterbi;
...
public class BSegmentViterbi extends SparseViterbi {
	...
	protected double getCorrectScore(DataSequence dataSeq, int i, int ell, double[] lambda) {
		...
		if ((segmentation.segmentEnd(segNum) != i) || ((segLength <= m) && (ell != segLength))
				|| ((segLength > m) && (ell != 1)))
			return 0;
		...
		double val = (Ri.getQuick(dataSeq.y(i)) + ((i - segLength >= 0) ? Mi
				.get(dataSeq.y(i - segLength), dataSeq.y(i)) : 0));
