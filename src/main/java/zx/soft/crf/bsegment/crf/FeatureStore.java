...
import zx.soft.crf.bsegment.crf.BSegmentTrainer.MatrixWithRange;
import zx.soft.crf.core.DataSequence;
import zx.soft.crf.core.Feature;
import zx.soft.crf.core.FeatureGenCache;
import zx.soft.crf.core.LogSparseDoubleMatrix1D;
import zx.soft.crf.core.RobustMath;
...
public class FeatureStore {
	...
	class FeatureImpl extends BasicEdgeFeature implements BFeature {
		...
		@Override
		public String toString() {
			return bfgen.featureName(_index) + " v" + value() + " y" + y() + "  s" + start() + ":" + startOpen()
					+ "  e" + end() + ":" + endOpen();
		}
	...
	class EdgeFeatures {
		...
		FeatureGenCache.AllFeatureCache.FeatureVector get(int i) {
			return edgeFeatureArray[i];
		}
	...
	private void addFeatures(DoubleMatrix1D mat, int type, int index, Condition predicate, boolean add) {
		if ((type == endExact) || (type == endOpen)) {
			...
			for (int s = startLB; s <= startUB; s++) {
				if (index - s >= 0)
					addFeatures(stateFeatures[t][index - s][s], mat, add);
			}
			if (!cond.openOnly) {
				t = (type == endOpen) ? 1 : 0;
				int startB = cond.val;
			...
			for (int e = endLB; e <= endUB; e++) {
				if (e - index >= 0)
					addFeatures(stateFeatures[t][e - index][index], mat, add);
			}
			if (!cond.openOnly) {
	...
	public DoubleMatrix1D getExactR(int s, int e, DoubleMatrix1D ri_Y, boolean endIsExact) {
		...
		if (endIsExact)
			addFeatures(ri_Y, endExact, e, cond);
		for (int i = e; i >= s; i--)
