...
public class AStarSearch {
	...
	*/
	...
	public AStarSearch(BoundUpdate boundUpdate, int avgStatePerExpansion, long maxExpansions, long maxQueueSize,
			boolean profiling) {
	...
	public State performAStarSearch(State startState) {
		...
		//System.out.print(numExpansions + "\t" + stateQueue.size() + "\t");
		if (debug && profiling)
			System.out.println("NumExpansions:" + numExpansions + " " + (Math.log(numExpansions) / Math.log(2)) + " :"
					+ stateQueue.size() + " " + (Math.log(stateQueue.size()) / Math.log(2)));
	...
	private boolean shouldUpdateBound() {
		return (boundUpdate != null && (numExpansions % curLowBoundCalStep == 0) && (numExpansions
				* avgStatesPerExpansion < stateQueue.size() || stateQueue.size() > maxQueueSize));
	}
