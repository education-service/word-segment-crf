...
import zx.soft.crf.bsegment.BFeatureGenImpl;
import zx.soft.crf.bsegment.crf.BSegmentCRF;
import zx.soft.crf.core.CRF;
import zx.soft.crf.core.FeatureGenerator;
import zx.soft.crf.core.NestedCRF;
import zx.soft.crf.core.SegmentDataSequence;
import zx.soft.crf.core.Segmentation;
import zx.soft.crf.core.Util;
import zx.soft.crf.model.FeatureGenImpl;
import zx.soft.crf.model.NestedFeatureGenImpl;
...
public class Segment {
	...
	public void train() throws Exception {
		...
		dir.mkdirs();
		TrainData trainData = DataCruncher.readTagged(nlabels, baseDir + "/data/" + inName + "/" + inName + ".train",
				baseDir + "/data/" + inName + "/" + inName + ".train", delimit, tagDelimit, impDelimit, labelMap);
	...
	public void doTest() throws Exception {
		...
		dir.mkdirs();
		TestData testData = new TestData(baseDir + "/data/" + inName + "/" + inName + ".test", delimit, impDelimit,
				groupDelimit);
		TestDataWrite tdw = new TestDataWrite(baseDir + "/out/" + outDir + "/" + inName + ".test", baseDir + "/data/"
				+ inName + "/" + inName + ".test", delimit, tagDelimit, impDelimit, labelMap);
	...
	public void calc() throws Exception {
		...
		TrainData tdMan = DataCruncher.readTagged(nlabels, baseDir + "/data/" + inName + "/" + inName + ".test",
				baseDir + "/data/" + inName + "/" + inName + ".test", delimit, tagDelimit, impDelimit, labelMap);
		TrainData tdAuto = DataCruncher.readTagged(nlabels, baseDir + "/out/" + outDir + "/" + inName + ".test",
				baseDir + "/data/" + inName + "/" + inName + ".test", delimit, tagDelimit, impDelimit, labelMap);
		...
		System.out.println("Ov:\t" + truePos[nlabels] + "\t" + totalMarkedPos[nlabels] + "\t" + totalPos[nlabels]
