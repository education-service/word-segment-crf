...
public class WordsInTrain implements Serializable {
	...
	public void read(BufferedReader in, int numStates) throws IOException {
		...
		for (int l = 0; (l < dictLen) && ((line = in.readLine()) != null); l++) {
			...
			while (entry.hasMoreTokens()) {
				StringTokenizer scp = new StringTokenizer(entry.nextToken(), FileFormat.WORD_STATE_SEPARATOR);
	...
	public void write(PrintWriter out) throws IOException {
		...
		for (Enumeration e = dictionary.keys(); e.hasMoreElements();) {
			...
			for (int s = nextStateWithWord(pos, -1); s != -1; s = nextStateWithWord(pos, s)) {
				out.print(FileFormat.WORD_ELEMENT_SEPARATOR + s + FileFormat.WORD_STATE_SEPARATOR
						+ getStateArray(pos)[s]);
			}
			out.println("");
		}
