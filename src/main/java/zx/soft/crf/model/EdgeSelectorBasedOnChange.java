...
public class EdgeSelectorBasedOnChange extends EdgeSelector {
	...
	public EdgeSelectorBasedOnChange(FeatureGenImpl fgen, int width, String patternFile, int histSize, int minsize) {
	...
	public EdgeSelectorBasedOnChange(FeatureGenImpl fgen, String patternFile, int histSize) {
	...
	protected boolean advance() {
		while (++index < (patternOccurence.length)
				&& (patternOccurence[index] <= 0 || patternOccurence[index] == segLen))
			;
