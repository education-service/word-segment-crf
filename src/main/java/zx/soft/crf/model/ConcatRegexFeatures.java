...
public class ConcatRegexFeatures extends FeatureTypes {
	...
	String patternString[][] = {
			...
			{ "singleDigit", "\\s*\\d\\s*" }, { "twoDigits", "\\s*\\d{2}\\s*" }, { "threeDigits", "\\s*\\d{3}\\s*" },
			{ "fourDigits", "\\s*\\(*\\d{4}\\)*\\s*" }, { "isDigits", "\\d+" }, { "containsDigit", ".*\\d+.*" },
			{ "isNumberRange", "\\d+\\s*([-]{1,2}\\s*\\d+)?" },

			{ "endsWithDot", "\\p{Alnum}+\\." }, { "endsWithComma", "\\w+[,]" },
			{ "endsWithPunctuation", "\\w+[;:,.?!]" }, { "singlePunctuation", "\\p{Punct}" }, { "singleAmp", "[&]" },

			{ "isDashSeparatedWords", "(\\w[-])+\\w" },
			{ "isDashSeparatedSeq", "((\\p{Alpha}+|\\p{Digit}+)[-])+(\\p{Alpha}+|\\p{Digit}+)" },
			{ "isURL", "\\p{Alpha}+://(\\w+\\.)\\w+(:(\\d{2}|\\d{4}))?(/\\w+)*(/|(/\\w+\\.\\w+))?" },
			{ "isEmailId", "\\w+@(\\w+\\.)+\\w+" }, { "containsDashes", ".*--.*" },
	 ...
	 */
	public ConcatRegexFeatures(FeatureGenImpl fgen, int relSegmentStart, int relSegmentEnd, int maxMemory,
			String patternFile) {
		super(fgen);
