...
public class RegexCountFeatures extends FeatureTypes {
	...
	String patternString[][] = {
			...
			{ "singleCapLetter", "[A-Z]" }, { "containsDashes", ".*--.*" },
			{ "containsDash", ".*\\-.*" },
			//{"singlePunctuation", 		"\\p{Punct}"			},
			{ "singleDot", "[.]" }, { "singleComma", "[,]" }, { "singleQuote", "[']" },
			{ "isSpecialCharacter", "[#;:\\-/<>'\"()&]" }, { "fourDigits", "\\d\\d\\d\\d" }, { "isDigits", "\\d+" },
			{ "containsDigit", ".*\\d+.*" }, { "endsWithDot", "\\p{Alnum}+\\." } };
	Pattern p[];
