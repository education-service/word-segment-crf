...
public class FeatureGenImpl implements FeatureGeneratorNested {
	...
	protected boolean retainFeature(DataSequence seq, FeatureImpl f) {
		return ((seq.y(cposEnd) == f.y() || seq.y(cposEnd) < 0) && ((cposStart == 0) || (f.yprev() < 0)
				|| (seq.y(cposStart - 1) == f.yprev()) || seq.y(cposStart - 1) < 0));
	}
		...
		public int getId(FeatureImpl f) {
			...
			if ((id >= 0) && featureCollectMode) {
				...
				if (!storedFIdentifier.name.equals(f.identifier().name)) {
					System.out.println("WARNING: same feature-id for different feature names?: " + storedFIdentifier
							+ ":" + f.identifier());
				}
			}
	...
	public static boolean featureValidStatic(DataSequence data, int cposStart, int cposEnd,
			FeatureImpl featureToReturn, Model model, boolean cacheEdgeFeatures) {
		if (((cposStart > 0) && (cposEnd < data.length() - 1)) || (featureToReturn.y() >= model.numStates())
