...
public abstract class FeatureTypes implements Serializable {
	...
	public int maxFeatureId() {
		zx.soft.crf.utils.LogMessage.issueWarning("WARNING : Class " + getClass().getName()
				+ " does not implement maxFeatureId(). Returning default value. Please refer to the documentation.");
	...
	public boolean featureCollectMode() {
		return fgen.featureCollectMode;
	}
	...
	public void train(DataSequence data, int pos) {
		if (requiresTraining()) {
			zx.soft.crf.utils.LogMessage
					.issueWarning("WARNING : Class "
							+ getClass().getName()
							+ " does not implement the train(DataSequence, int) method. Please implement the train() methods properly.");
		}
	}
	...
	public void train(SegmentDataSequence sequence, int segStart, int segEnd) {
		if (requiresTraining()) {
			zx.soft.crf.utils.LogMessage
					.issueWarning("WARNING : Class "
							+ getClass().getName()
							+ " does not implement the train(SegmentDataSequence, int, int) method. Calling train(DataSequence, int) instead. Please implement the train() methods properly.");
	...
	public void trainingDone() {
		if (requiresTraining()) {
			zx.soft.crf.utils.LogMessage
					.issueWarning("WARNING : Class "
							+ getClass().getName()
							+ " does not implement the train(SegmentDataSequence, int, int) method. Calling train(DataSequence, int) instead. Please implement the trainDone() methods properly.");
		}
	}
